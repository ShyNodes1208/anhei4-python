--========================================================
-- 暗黑破坏神4：G502 三合一塑形脚本
-- 分辨率：2560 × 1440
--
-- 每组处理3件装备：
-- 选择“三合一塑形”
-- → 右键连续3件装备
-- → 点击“重塑”
-- → 右键取回产物
-- → 等待2秒
-- → 处理下一组
--========================================================

--==================== 基础配置 ====================

local SCREEN_W = 2560
local SCREEN_H = 1440

-- G502按键编号
local START_BUTTON = 5
local STOP_BUTTON  = 4

-- 指定背包操作范围
local ROWS = 3
local COLS = 9

-- 每组需要放入的装备数量
local ITEMS_PER_GROUP = 3

--==================== 背包坐标 ====================

-- 第一行第一格中心
local BAG_START_X = 1722
local BAG_START_Y = 1017

-- 相邻格子中心距离
local BAG_STEP_X = 73
local BAG_STEP_Y = 102

--==================== 魔盒坐标 ====================

-- 配方列表中的“三合一塑形”
local THREE_IN_ONE_X = 1140
local THREE_IN_ONE_Y = 930

-- 左侧下方“重塑”按钮
local REFORGE_X = 468
local REFORGE_Y = 1140

-- 魔盒第一个装备格/产物位置
local CUBE_ITEM_X = 322
local CUBE_ITEM_Y = 480

-- 鼠标安全停留位置
local SAFE_X = 2103
local SAFE_Y = 929

--==================== 时间配置 ====================

-- 按启动键后等待2秒
local START_DELAY = 2000

-- 鼠标移动到位置后等待
local MOVE_DELAY = 200

-- 每次操作后等待1秒
local RECIPE_DELAY = 1000
local BAG_CLICK_DELAY = 1000
local REFORGE_DELAY = 1000
local RETURN_DELAY = 1000

-- 每组完成后等待2秒
local BETWEEN_GROUPS_DELAY = 2000

--==================== 运行状态 ====================

local running = false

--========================================================
-- 像素坐标转换成G HUB绝对坐标
--========================================================

function MoveToPixel(x, y)
    local ghubX = math.floor(
        x * 65535 / (SCREEN_W - 1)
    )

    local ghubY = math.floor(
        y * 65535 / (SCREEN_H - 1)
    )

    MoveMouseTo(ghubX, ghubY)
end

--========================================================
-- 根据格子序号计算背包坐标
--
-- index范围：0～32
-- 顺序：从左到右、从上到下
--========================================================

function GetBagSlotPosition(index)
    local row = math.floor(index / COLS)
    local col = index % COLS

    local x = BAG_START_X + col * BAG_STEP_X
    local y = BAG_START_Y + row * BAG_STEP_Y

    -- 第11列按原脚本向上微调1像素
    if col == COLS - 1 then
        y = y - 1
    end

    return x, y
end

--========================================================
-- 检查停止键
--========================================================

function StopRequested()
    return IsMouseButtonPressed(STOP_BUTTON)
end

--========================================================
-- 可中断等待
--========================================================

function SleepChecked(milliseconds)
    local elapsed = 0
    local interval = 25

    while elapsed < milliseconds do
        if StopRequested() then
            return false
        end

        local sleepTime = interval

        if elapsed + sleepTime > milliseconds then
            sleepTime = milliseconds - elapsed
        end

        Sleep(sleepTime)
        elapsed = elapsed + sleepTime
    end

    return true
end

--========================================================
-- 移动鼠标并点击
--
-- mouseButton：
-- 1 = 左键
-- 2 = 中键
-- 3 = 右键
--========================================================

function ClickAt(x, y, mouseButton, afterDelay)
    if StopRequested() then
        return false
    end

    MoveToPixel(x, y)

    if not SleepChecked(MOVE_DELAY) then
        return false
    end

    PressAndReleaseMouseButton(mouseButton)

    return SleepChecked(afterDelay)
end

function MoveToSafePosition()
    MoveToPixel(SAFE_X, SAFE_Y)
end

--========================================================
-- 处理一组三件装备
--========================================================

function ProcessOneGroup(groupNumber, firstSlotIndex)
    OutputLogMessage(
        "开始处理第%d组，背包格子：%d、%d、%d。\n",
        groupNumber,
        firstSlotIndex + 1,
        firstSlotIndex + 2,
        firstSlotIndex + 3
    )

    -- 1. 点击“三合一塑形”
    if not ClickAt(
        THREE_IN_ONE_X,
        THREE_IN_ONE_Y,
        1,
        RECIPE_DELAY
    ) then
        return false
    end

    -- 2. 依次右键3件背包装备
    for offset = 0, ITEMS_PER_GROUP - 1 do
        local slotIndex = firstSlotIndex + offset
        local bagX, bagY = GetBagSlotPosition(slotIndex)

        OutputLogMessage(
            "右键第%d格，坐标=%d,%d。\n",
            slotIndex + 1,
            bagX,
            bagY
        )

        if not ClickAt(
            bagX,
            bagY,
            3,
            BAG_CLICK_DELAY
        ) then
            return false
        end
    end

    -- 3. 点击“重塑”
    if not ClickAt(
        REFORGE_X,
        REFORGE_Y,
        1,
        REFORGE_DELAY
    ) then
        return false
    end

    -- 4. 右键取回产物
    if not ClickAt(
        CUBE_ITEM_X,
        CUBE_ITEM_Y,
        3,
        RETURN_DELAY
    ) then
        return false
    end

    MoveToSafePosition()

    OutputLogMessage(
        "第%d组处理完成，等待2秒。\n",
        groupNumber
    )

    return SleepChecked(BETWEEN_GROUPS_DELAY)
end

--========================================================
-- 处理指定范围内全部装备
--========================================================

function RunAllGroups()
    local totalSlots = ROWS * COLS
    local totalGroups = math.floor(
        totalSlots / ITEMS_PER_GROUP
    )

    OutputLogMessage(
        "开始三合一塑形：共%d格，分为%d组。\n",
        totalSlots,
        totalGroups
    )

    for groupIndex = 0, totalGroups - 1 do
        if StopRequested() then
            MoveToSafePosition()
            OutputLogMessage("检测到停止键。\n")
            return false
        end

        local firstSlotIndex =
            groupIndex * ITEMS_PER_GROUP

        local success = ProcessOneGroup(
            groupIndex + 1,
            firstSlotIndex
        )

        if not success then
            MoveToSafePosition()

            OutputLogMessage(
                "脚本在第%d组停止。\n",
                groupIndex + 1
            )

            return false
        end
    end

    MoveToSafePosition()

    OutputLogMessage(
        "三合一塑形完成，共处理%d组、%d件装备。\n",
        totalGroups,
        totalGroups * ITEMS_PER_GROUP
    )

    return true
end

--========================================================
-- 启动程序
--========================================================

function StartThreeInOne()
    if running then
        OutputLogMessage(
            "脚本正在运行，忽略重复启动。\n"
        )
        return
    end

    running = true

    OutputLogMessage(
        "脚本将在2秒后开始执行。\n"
    )

    if not SleepChecked(START_DELAY) then
        running = false
        MoveToSafePosition()
        OutputLogMessage("启动已取消。\n")
        return
    end

    RunAllGroups()

    running = false
    MoveToSafePosition()
end

--========================================================
-- G HUB事件入口
--========================================================

function OnEvent(event, arg)
    OutputLogMessage(
        "事件：%s，按键编号：%s\n",
        tostring(event),
        tostring(arg)
    )

    if event == "PROFILE_ACTIVATED" then
        EnablePrimaryMouseButtonEvents(true)

        OutputLogMessage(
            "暗黑4三合一塑形脚本已加载。\n"
        )
    end

    if event == "MOUSE_BUTTON_PRESSED"
        and arg == START_BUTTON
        and not running then

        StartThreeInOne()
    end
end